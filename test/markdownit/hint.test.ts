...
describe('hint renderer', () => {
  it('wraps the body in a styled callout', () => {
    const html = renderLikeVsCode(md, '{% hint style="warning" %}\nBe careful.\n{% endhint %}');
  ...
  it('defaults to the info style when none is given', () => {
    expect(renderLikeVsCode(md, '{% hint %}\nx\n{% endhint %}')).toContain('gb-hint--info');
  });

  it('falls back to info for an unrecognized style', () => {
    expect(renderLikeVsCode(md, '{% hint style="nope" %}\nx\n{% endhint %}')).toContain('gb-hint--info');
  });

  it('renders markdown inside the body', () => {
    const html = renderLikeVsCode(md, '{% hint style="info" %}\nUse **bold** and `code`.\n{% endhint %}');
  ...
  it('leaves an unknown tag as literal text', () => {
    expect(renderLikeVsCode(md, '{% bogus %}')).toContain('{% bogus %}');
  });

  it('does not treat an indented code block as a tag', () => {
    expect(renderLikeVsCode(md, '    {% hint %}')).toContain('<pre><code>{% hint %}');
  });
});
